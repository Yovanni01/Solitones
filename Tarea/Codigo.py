...
def shooting(df0f, df0i, nu, ri, f0, sistem, NP, h):
            
    while True:
        df0 = (df0f+df0i)/2; #Obtenciaon de alguna derivada inicial de prueba
        Cond_0 = [f0, df0]; #Condiciones iniciales
        fs,x = RK4V(NP,h,sistem,Cond_0,ri); #Solucion con derivada inicial de prueba
        
        if abs(fs[-1,0])-nu < 0: # Evaluacion del signo
            df0f = df0
            
        elif abs(fs[-1,0])-nu > 0: # Evaluacion del signo
            df0i = df0
            
        elif abs(fs[-1,0])-nu < 1e-14: # Prueba de tolerancia
            return df0
        
        if abs((df0f-df0i)/2) < 1e-14:      # Prueba de tolerancia
            return df0
        
        #print(fs[-1,0]);

def shooting2(df0f, df0i, l, V, n, rf, ri, f0, sistem, NP, h):
            
    while True:
        df0 = (df0f+df0i)/2; #Obtenciaon de alguna derivada inicial de prueba
        Cond_0 = [df0,f0]; #Condiciones iniciales
        fs,x = RK4V(NP,h,sistem,Cond_0,ri); #Solucion con derivada inicial de prueba
        
        if abs(fs[-1,0])-V < 0: # Evaluacion del signo
            df0f = df0
            #print('sube', abs(sol_.y[0][-1]),df0min)
            
        elif abs(fs[-1,0])-V > 0: # Evaluacion del signo
            df0i = df0
            #print('baja', abs(sol_.y[0][-1]),df0max)
            
        elif abs(fs[-1,0])-V < 1e-14: # Prueba de tolerancia
            #print('Hola3')
            return df0
...
# Ejercicio 3, tarea 2. Codigo para 1+2 dimensiones
def Dif_Fin_2(n,L,Nx,Nt,tf,U0,dU0,DerPot,CondicionF,Name1):
    dx=L/Nx;
    x0=0;
    xf=L;
    t0=0;
    ...
    for i in range (1,Nx-1):
        u[i]=U0[i]+dt*dU0[i]+(1/2)*(C**2)*(U0[i+1]-2*U0[i]+U0[i-1])+(1/2)*(dt**2)*(-DerPot(U0[i])+((1/xi[i])*(dU0[i])-(n**2)*(U0[i])/(xi[i]**2)));  
    u[0]=CondicionF(x0,t[1]);
    u[-1]=CondicionF(xf,t[1]);
    ...
    for j in range (0,Nt):
        for i in range (1,Nx):
            if j==0:
                U[i,j+1]=-U0[i]+2*u[i]+(C**2)*(u[i+1]-2*u[i]+u[i-1])+(dt**2)*(-DerPot(u[i])+((1/xi[i])*((u[i+1]-u[i-1])/2*dx)-(n**2)*(u[i])/(xi[i]**2))); #-U[i,j-1]+2*U[i,j]+(C**2)*(U[i+1,j]-2*U[i,j]+U[i-1,j])+(dt**2)*(DerPot(U[i,j]));
            else:
                U[i,j+1]=-U[i,j-1]+2*U[i,j]+(C**2)*(U[i+1,j]-2*U[i,j]+U[i-1,j])+(dt**2)*(-DerPot(U[i,j])+((1/xi[i])*((U[i+1,j]-U[i-1,j])/2*dx)-(n**2)*(U[i,j])/(xi[i]**2)));
        U[0,j+1]=CondicionF(x0,t[j]);
        U[-1,j+1]=CondicionF(xf,t[j]);
...
# Ejercicio 3, tarea 2. Codigo para 1+3 dimensiones
def Dif_Fin_3(L,Nx,Nt,tf,U0,dU0,DerPot,CondicionF,Name1):
    dx=L/Nx;
    x0=0;
    xf=L;
    t0=0;
    ...
    u=np.zeros(Nx+1);    
    for i in range (1,Nx-1):
        u[i]=U0[i]+dt*dU0[i]+(1/2)*(C**2)*(U0[i+1]-2*U0[i]+U0[i-1])+(1/2)*(dt**2)*(-DerPot(U0[i])+((2/xi[i])*(dU0[i])-(2)*(U0[i])/(xi[i]**2)));  
    u[0]=CondicionF(x0,t[1]);
    u[-1]=CondicionF(xf,t[1]);
    ...
    for j in range (0,Nt):
        for i in range (1,Nx):
            if j==0:
                U[i,j+1]=-U0[i]+2*u[i]+(C**2)*(u[i+1]-2*u[i]+u[i-1])+(dt**2)*(-DerPot(u[i])+((2/xi[i])*((u[i+1]-u[i-1])/2*dx)-(2)*(u[i])/(xi[i]**2))); #-U[i,j-1]+2*U[i,j]+(C**2)*(U[i+1,j]-2*U[i,j]+U[i-1,j])+(dt**2)*(DerPot(U[i,j]));
            else:
                U[i,j+1]=-U[i,j-1]+2*U[i,j]+(C**2)*(U[i+1,j]-2*U[i,j]+U[i-1,j])+(dt**2)*(-DerPot(U[i,j])+((2/xi[i])*((U[i+1,j]-U[i-1,j])/2*dx)-(2)*(U[i,j])/(xi[i]**2)));
        U[0,j+1]=CondicionF(x0,t[j]);
        U[-1,j+1]=CondicionF(xf,t[j]);
